"""OQ18 Task A-prime: NAME the fit-phase transient that sets the production peak.
...
Usage:
    oq18-aprime-harness.py <workdir> <side> <live> <chunk> <target_s> <trace>
                           [tag] [n_time] [candidate_set]
...
def main() -> None:
    ...
    name = f"s{side}_l{live}_{chunking}_t{int(target)}_n{n_time}_{which}_x{trace}_{tag}"
