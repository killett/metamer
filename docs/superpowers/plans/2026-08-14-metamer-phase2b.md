...
| open | closer | machine |
|---|---|---|
| **OQ18** — the budget bounds the peak (Q1) and the tile is sized from a model of residency (8b) | decide what the budget bounds or make the two agree. ~~first hypothesis is a pipeline change, freeing the block before the store write~~ — **run 2026-08-19 as Task A**: the peak is `max(fit-phase transient, post-write plateau)` and which wins crosses over in B. The free takes the peak down by nearly the whole block where the write's plateau is the peak, and cannot reach it at production B, where the maximum is attained inside `fit` with the block alive; criterion 7 fails in both arms. The peak's *location* is now instrumented per phase, which was the second closer. **Next is naming the fit transient — cheaper than the crossed 2 × 2 and ahead of it** — and the 2 × 2, never an L, must report **per phase** rather than a run maximum | this one; the masked duration-controlled ladder is ~30 s a point |
