...
# invalidated in practice; this is the second and last such regeneration
...
GOLDEN_FIT_HASH = "faf2d107bab48b06"
GOLDEN_COMPAT_HASH = "bb28cb8d4bffa049"
GOLDEN_RUN_HASH = "af313190251af95f"


def _config(**overrides):
    """A fully specified config: every allowlisted field a caller supplies.

    `algorithm_version` is deliberately absent: `normalize` stamps it from the
    installed code and refuses a config that carries it. `metamer_version` is
    present and is provenance only -- it reaches `run_hash` and neither of the
    other two.
    """
    base = {
...
def test_bumping_the_algorithm_version_moves_both_gates(monkeypatch):
    """A declared algorithm change invalidates fits and derived arrays alike.

    Expected value determined independently: `ALGORITHM_VERSION` is in
    `FIT_RELEVANT_FIELDS`, and `COMPAT_RELEVANT_FIELDS` is a strict superset,
    so moving it must move both. This is the parametrized
    `test_a_fit_mismatch_always_forces_a_compat_mismatch` case for the one
    field that cannot be varied through a config, exercised through the
    mechanism that actually varies it.

    Bug this catches: stamping the constant somewhere `_subset` does not read,
    or filing it under a key the allowlist does not name. Either leaves a
    constant that looks authoritative in the source, is documented as the gate
    on stored fits, and gates nothing -- so bumping it after a change to the
    objective silently reuses every fit it was bumped to invalidate.
    """
    before = (fit_hash(_config()), compat_hash(_config()), run_hash(_config()))
    monkeypatch.setattr(hashing, "ALGORITHM_VERSION", "2")
...
def test_a_config_supplying_the_algorithm_version_is_refused():
    """Algorithm identity comes from the code, and a config cannot claim it.

    Expected value determined independently: the constant is a property of the
    installed code. A config value for it would be a claim about code the
    config cannot see, and the reserved-key refusal is the same construction
    ...
    -- reasonably, having read that it gates resumption -- would then have it
    accepted and ignored, and would believe they had pinned something they had
    not. That is the "silent skip in an allowlist is a demotion" failure
    pointed the other way: a silent OVERRIDE of an explicit request.
    """
    with pytest.raises(ValueError, match="algorithm_version"):
        fit_hash(_config(algorithm_version="99"))
    with pytest.raises(ValueError, match="algorithm_version"):
        run_hash(_config(algorithm_version="99"))
    assert fit_payload(_config())["algorithm_version"] == ALGORITHM_VERSION
...
@pytest.mark.parametrize(
    "field", sorted(FIT_RELEVANT_FIELDS - {hashing.ALGORITHM_VERSION_KEY})
)
def test_a_fit_mismatch_always_forces_a_compat_mismatch(field):
    ...
    `algorithm_version` is excluded from the parametrization because a config
    cannot carry it -- `normalize` refuses one that does. It is covered by
    `test_bumping_the_algorithm_version_moves_both_gates`, which moves the
    module constant instead, i.e. exercises the real mechanism rather than a
    config field the mechanism forbids.
    """
    changed = _config(
