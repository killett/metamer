...
3. **NEXT ACTION: 2f Task 3 — the drop row, and 2e's criterion 14.** Task 2 is DONE: the ruling of 2026-09-22 gave the coverage denominator **its own predicate**, `Outcome.is_covered`, rather than an exclusion at the call site, and the four predicates now nest and are readable as one table in `core/outcomes.py`. The plan is [`2026-09-19-metamer-phase2f.md`](docs/superpowers/plans/2026-09-19-metamer-phase2f.md), **APPROVED with ten review amendments**; the pre-flight is [`phase2f-preflight.md`](docs/superpowers/notes/phase2f-preflight.md) and every task's entry is written **before** its code. **Task 0 (the clustering spike) and Task 1 (the reader) are DONE.** The queued follow-up is [here](#the-queued-follow-up--2fs-next-commit) and is **SPENT — all five items closed, items 2 and 4 by the follow-up commit.** Both rulings are verbatim at the pre-flight, with an addendum recording the three things the code found that they did not cover.
