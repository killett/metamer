...
| **Phase 2e implementation plan** | [`docs/superpowers/plans/2026-09-12-metamer-phase2e.md`](docs/superpowers/plans/2026-09-12-metamer-phase2e.md) — 8 tasks, 17 exit criteria **each naming its reading**, **AWAITING REVIEW; NO CODE YET** (the head above is the single source for this status). Implements design doc **§14.1 and §14.3 only** — §14.2 is 2f's, and the split is §17's measure/print rule applied. Carries fifteen decisions with their reasons, three corrections it owes to the tree and to this file, and the one-pass abort gap as a decision the sub-phase must take. Task 1 precedes Task 2 by (i2) and that ordering is not negotiable by convenience |
