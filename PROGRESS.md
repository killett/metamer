...
3. **NEXT ACTION: 2f Task 4 — the primitives sections, and the run's resolved-candidate record.** Tasks 2 and 3 are DONE. **TASK 4 OWES THE RECORDED DENOMINATOR NAME** (the verdict writes which population it thresholded on; inferred from field presence until then, with the `49f3db1`–`f4eb42f` ambiguity stated at `gate_denominator`). **THREE FURTHER OBLIGATIONS ARE OWED AT TASK 10** and are listed in the open items below; they are the price of 2f having no criteria suite yet. The plan is [`2026-09-19-metamer-phase2f.md`](docs/superpowers/plans/2026-09-19-metamer-phase2f.md), **APPROVED with ten review amendments**; the pre-flight is [`phase2f-preflight.md`](docs/superpowers/notes/phase2f-preflight.md) and every task's entry is written **before** its code. **Task 0 (the clustering spike) and Task 1 (the reader) are DONE.** The queued follow-up is [here](#the-queued-follow-up--2fs-next-commit) and is **SPENT — all five items closed, items 2 and 4 by the follow-up commit.** Both rulings are verbatim at the pre-flight, with an addendum recording the three things the code found that they did not cover.
   ...
   > | 2 | [the plan](docs/superpowers/plans/2026-09-19-metamer-phase2f.md)'s **Task 3** and **D11** | the brief the code is written against |
...
5. **Tests: 1539 passed, 0 failed, 0 INDETERMINATE — 2026-09-23, 5272.31 s (1:27:52), exit code pytest's own** (1535 at Task 3; 1525 at Task 2; 1509 at the follow-up; 1506 at the reconciliation, 1499 at 2f Task 1, 1486 at the OQ24 flip, 1483 at 2e's close). **The +4 are Task 3's defect fix — the drop row showed a rate the gate never used.** **AND TASK 2's FIRST SWEEP FAILED, ALONE, IN NEITHER FILE THE TARGETED RUNS COVERED** — a renamed test that a CLOSED 2e criterion named as its evidence; see the gotcha and the pre-flight. **The +3 over 1506 are the follow-up's: the golden table, the run-time boundary reading, and its control.** **TWO SWEEPS ON AN IDENTICAL TREE, A DAY APART: 1:13:45 and 1:15:34, and the stall diagnostic read 0 above threshold in one and 2 in the other** — a second dated instance of the variance the gotcha below records, and the reason a duration is never evidence about code. Both are far under the 2:00:31 measured on a SMALLER tree. **WRITERS RUN BEFORE THE SWEEP AND THE SWEEP RUNS LAST**, with `git write-tree` recorded at sweep start and asserted equal at commit time — a lint autofix once rewrote bytes a 78-minute sweep had already described. Older figures: ~~1483 passed, 2026-09-18, 5745.87 s (1:35:45)~~ (1482 earlier the same day with eighteen criteria, 1465 after the no-evidence decision, 1463 on 2026-09-16; the one added last is criterion 19's test). `pixi run test` is the full sweep and the only end-of-task evidence. **The run an hour earlier on the IDENTICAL TREE took 1:40:14, had 5 above the diagnostic, and printed a floor ladder 32% low and non-monotonic — see the gotcha. Same code, same fixture, same box.** **AND THE EXIT CODE READ IS PYTEST's OWN.** A `pixi run test 2>&1 | tail -25` on the run before this one reported **exit 0 with a test failing** — the code was `tail`'s. Handoff §2's rule, third instance: *where a tool reports on something else, its exit code describes the tool.* **Run the sweep to a file and read its status; never pipe it.** **`pixi run test` is the full sweep and every end-of-task verification must run it; `test-fast` and `test-ci` are NOT evidence** — the full sweep has caught **nine** things a fast run could not, the most recent being 2f Task 2's rename of a test that a CLOSED 2e criterion named as its evidence — the second breakage of that same name, which is why the count came out of it. **Every run prints `RSS measurement validity`, including at zero**; a nonzero count is INDETERMINATE, neither pass nor fail.
...
| — | **the queued follow-up: item 2's golden table and item 4's run-time probe** (`3694ae0`) |
| 2 — rates, both denominators | **DONE.** `is_covered` added as the coverage denominator's own predicate; both rates are fields of `failure_tally`; `report/numbers.py` computes nothing |
| 3 — the drop row, and 2e's criterion 14 | **DONE, plus a DEFECT FIX**: the row showed a rate the gate never used — see the pre-flight. **DONE.** The row is over pass 1's population, recomputed when the sibling store is present and labelled carried when it is not; **2e's 14 does NOT flip** — forward pointer instead |
| 4–10 | planned |
...
**ALL FIVE ARE NOW CLOSED.** Items 1, 3 and 5 were closed by the reconciliation; **items 2 and 4 landed as code at the follow-up commit** and are struck below, each naming where its deliverable now lives. **The queue is spent and Task 2 is not blocked by it.**
...
1. ~~**Re-aim the tautological agreement test** and `failure_tally`'s own tests at values computed
   ...
   never built.
2. ~~**Count the rate-computation enumeration per file or per computation.**~~ **DONE AT THE
   FOLLOW-UP COMMIT, AND THE UNIT WAS NEITHER OF THE TWO THE ITEM OFFERED.** *"Five sites"* and
   *"fifteen"* are **deleted, not maintained** — the ruling was *don't count what you can't
   classify; pin what you can observe.* The deliverable is a golden table in
   `tests/test_outcomes.py`: scope is every `src/` module that references `Outcome`, the `ast`
   decides which, and the test asserts **the set of those modules and each module's division-node
   count**, classifying nothing. **The table is there and is not copied here.** Frozen instruments
   under `notes/` are excluded by the frozen-instrument rule, with the exclusion and its reason
   stated in the test, and **the two gaps are named in the test** — the suite does not enforce that
   a new harness calls `failure_tally`, and a rate over raw integer codes never referencing
   `Outcome` is outside the scope entirely. `audit_report`'s *"two quantities"* is corrected to
   **one `_rate` division serving seven callers** in the same commit; the denominator itself stays
   filed as **OQ25**. The ruling is verbatim at the pre-flight, and what the code turned up beyond
   it is the pre-flight's closing addendum.
...
4. ~~**The run-time import probe and the ceiling.**~~ **DONE AT THE FOLLOW-UP COMMIT.** The probe
   now **imports and CALLS `read_store`** on a fixture built in the parent, with the import-time
   probe kept as the cheap first check and relabelled as such — it stops at 65 modules and
   establishes nothing on its own. The ceiling is **980**, 5% above the measured 933, **as a
   backstop for UNNAMED dependencies rather than a second copy of the denylist**: `pydantic` and
   `metamer.batch.run` are caught by name, so `batch.run`'s 1002 is not the figure it discriminates
   against. A permanent positive control injects one import into the identical probe source and
   asserts both readings move. **CI's own count reaches the log through
   `conftest.DIAGNOSTIC_LINES`, not through the assertion message** — a message prints only when it
   fires, so a green run recorded nothing; that gap is the pre-flight addendum's first finding.
   **MEASURED 2026-09-22 ON THE FIRST GREEN RUN: four environments, four different counts, the
   ceiling unchanged because the margin is taken from the larger and that is the local one.**
   The figures are at the ceiling test's docstring and nowhere else; **a pin would have fired
   on three of the four passing runs**, which is pin-versus-bound measured rather than argued.
   **Criterion 3's record in the plan now states its evidence was vacuous from `3f04f68` until this
   commit**, along with the residual limit: a `sys.modules` check cannot see a lazy import by
   construction, which is what D10's maps design requires.
...
### ~~Task 2's accumulated amendments~~ — ALL FOUR DISCHARGED 2026-09-22

**Kept as the record of what the task owed, struck because it is delivered.** The fourth —
*"an absent `domain_mask` means the caveat applies"* — is still live as a **Task 4** obligation:
the field does not exist yet, so every store gets the caveat, and Task 4 is what makes the
absent branch reachable.
...
## 2e — HANDOFF AFTER TASK 6 (2026-09-16)

> **2e's Tasks 0–6 are done; Task 7 is not started.** Plan, pre-flight and every per-task finding
> are in [the plan](docs/superpowers/plans/2026-09-12-metamer-phase2e.md) and
...
**The early abort is two-pass only.** A probe pass with one job paying a full coarse fit was
rejected. **Consequence, which lives in the join between two sections:** exit 1's producer is the
verdict, so **a one-pass run can reach neither an early-abort 2 nor any 1.** Written at design doc
**§14.1** (the decision) **and §14.3's code-1 note** (because a script author reads the code table,
not the abort section). The one-pass store says so in its root attrs:
`early_abort = {"evaluated": false, "reason": "one-pass run: …"}`.

### 3. EXIT 1 HAS A DEFINITION NOW, AND THE DESIGN DOC NEVER GAVE IT ONE

*"Completed with failures above threshold"* named no threshold. **Definition, 2e's: a two-pass
run whose verdict found a candidate above §14.1's threshold, and which completed anyway** — under
`--on-candidate-failure=drop` or `continue`. No second threshold, no science policy; a whole-run
rate is §14.2's and 2f's. **The final console line carries the caveat that the rate is the COARSE
pass's** — without it, exit 1 reads as a statement about the run and it is a statement about a
sample. Code 2 now has two producers (preemption, abort), which the final line distinguishes and
`TwoPassReport.aborted` / `.interrupted` keep apart.

**What the drop guarantees, each asserted:** neither `fit_hash` nor `compat_hash` moves (else the
resumed run refuses its own store and §12.8's gate defeats itself); pass 1 keeps its real outcomes
(the evidence for the drop lives there); a second process reaches the same verdict (Task 5's
purity, asserted against the real verdict); the dropped candidate costs zero iterations (not
fitted-and-relabelled); and pass 2's root attrs record threshold, policy and population, because
the first two are CLI flags that exist nowhere else — **§17's measure/print rule**.

### 4. `| tail` HIDES FAILURES IN TWO WAYS, AND THIS SESSION HIT BOTH

**The exit code of a pipe is the last command's** — `pixi run test | tail -25` returned **0 with a
test failing** (Task 2). **And `tail -N` truncates what you READ** — `pre-commit run --all-files |
tail -4` showed three cheap hooks passing and **hid mypy's failure four lines up** (Task 5):
pre-commit prints one line per hook with failures in the middle, so `tail -N` structurally shows
the cheap ones and hides the expensive one. **Fix: run a sweep to a file and read its own status;
read hooks with `rg -e 'Failed|error:'`.** Both caught by something else — the summary read as
text, the commit hook — never by the command meant to check.

### 5. THE FLOOR LADDER IN THE SWEEP SUMMARY IS A DIAGNOSTIC, NOT A FLOOR

It is the **unasserted** `measure_floor` call; the banded one never prints. **Do not quote the
summary line as a floor.** The readings, the non-monotonic sweep, and the same-tree control that
confirmed it was the host are in [Gotchas](#gotchas-discovered) and are not restated here.

### 6. TWO FIXTURE GUARDS CAUGHT THEIR OWN FIXTURES BEFORE AN ASSERTION COULD LIE

Both are (i12)-shaped — a fixture that cannot express the condition its test names:

- **The duplicate-dims witness was unstorable.** `(2, 3, 4)` with dims `("time","x","x")` cannot be
  built — `xr.Dataset` refuses inconsistent sizes per name — so the first test died in its fixture
  and one of two reported symptoms was an artefact. The reachable witness has **equal** extents.
- **A 4×4 coarse grid cannot express 90%.** `0.9 × 16 = 14.4`; the boundary test's precondition
  refused to compare 0.875 against 0.9 and call it a boundary. The grid is now 20 points and says
  why.

**The pattern: when a test's subject is a boundary or a shape, assert first that the fixture can
express it.** Cheap, and it fired twice in one sub-phase.

### 7. FILED FORWARD

**Open question 24** (is `INSUFFICIENT_DATA` in the denominator — §8.6 stale, code follows it) is
**2f's**, with a dated superseded-note left at §8.6. **Open question 20** gained its second answer
(duplicate dimension names, closed by Task 2); coordinate direction is still open.

### 8. SELF-CORRECTIONS THIS SESSION, SO THE NEXT ONE DOES NOT RE-DERIVE THEM

The tiling-site sweep was incomplete **three times** (by paraphrase, by sentence shape, by describing
the effect with no count) — the count now lives in `tests/test_dimension_name_sites.py`, and
*"this defect is open"* has no such home. **Task 3's `Outcome` docstring named the wrong task**
(Task 1 for Task 3), corrected at Task 6. **A consequence noted in writing and not acted on** came
back 69 minutes later as a sweep failure (Task 2's handover had two sites). **mypy blocked two
commits** on test code a passing suite had accepted.
...
## Head items retired from the cold-start summary (2026-09-16)

> **MOVED, NOT COPIED.** These were head items 2, 3, 4, 6, 7, 8 and 9 until 2e's handoff; the head
> is now ten short lines that point here. **Nothing below is restated in the head.** Item 9's
> defect (a) — the tiling name-dependence — **is CLOSED** (2e Tasks 1 and 2); its text is kept
> because it is what made their ordering.
...
3. **NEXT ACTION: WIRING ONE — A DRIVER THAT RUNS `batch.audit` OVER A RUNG'S ARMS — AND IT IS URGENT RATHER THAN TIDY-UP.** Its brief, the two things the spike added to it, and the pieces already built that it reuses are in [WIRING ONE](#wiring-one--what-the-next-session-needs-written-2026-09-10). **WIRING ONE IS BUILT AND SMOKED, 2026-09-11 — [pre-flight](docs/superpowers/notes/wiring-one-preflight.md) (which found THREE disagreements between the brief and the committed records, one of them in this file), [smoke record](docs/superpowers/notes/wiring-one-smoke.md), both gate branches. CRITERION 12 STAYS REDUCED AND ITS CLOSER CHANGED — no rung was re-run, and that is a [decision with three reasons](#the-scope-call-on-criterion-12-taken-2026-09-11-validate-on-smoke-do-not-re-run-the-rung). THE SELECTION-MAP REPORT IS BUILT AND SMOKED (2026-09-11) — [pre-flight](docs/superpowers/notes/selection-map-report-preflight.md), [smoke record](docs/superpowers/notes/wiring-one-smoke.md). **`κ` PER ARM IS NOT A PRODUCTION STORE FIELD AND THE REASON IS STRUCTURAL: A STORE IS ONE ARM.** The byte count is a second fact beside it — and that ordering paid on the day it was written, because the size estimate turned out to be wrong by 2.2× and **the number that moved was the one that was not load-bearing.** **2e's BRAINSTORM IS DONE AND ITS PLAN IS WRITTEN, 2026-09-12 — [`2026-09-12-metamer-phase2e.md`](docs/superpowers/plans/2026-09-12-metamer-phase2e.md), 8 tasks, **18** exit criteria each naming its reading, **APPROVED 2026-09-12 with four amendments; Task 0 in progress, no production code yet.** **§14 IS SPLIT INTO TWO SUB-PHASES AND THE LINE IS §17's OWN MEASURE/PRINT RULE APPLIED, NOT A SCOPING PREFERENCE: 2e is §14.1 and §14.3 — the run's own honesty; 2f is §14.2 — the report.** Fifteen decisions, three corrections and one decision 2e must take are in the plan and **none is restated here.** The four that a cold session cannot re-derive: **the tiling closer is POSITIONAL THROUGHOUT on the FINGERPRINT argument** — `geometry_components` keys `spatial_coordinates` by the input's own dimension names and runs downstream of `open_input`, so a rename-at-adapter would make the store **assert a provenance its source file does not have**, which is a different category of defect from the documentation disagreement the grep was run to size; **`INTERNAL_ERROR` lands BEFORE the tiling closer on (i2)**, because `tiling.py`'s `KeyError('y')` is a live positive control available only until it is fixed, and the guard that survives carries **dated provenance** rather than being a mechanism checked against itself; **`CANDIDATE_DROPPED` moves into the decided-skip group with `SCREENED_OUT`**, and the rule — *a code the run assigns because the run chose not to fit is eligible and not a failure* — is recorded at `Outcome` so a future member is classified by the rule rather than by precedent; and **2f is LESS BLOCKED than "2e then 2f" reads** — stores, `/selection/delta_ic` and `audit_report.py` all exist today, and what 2f waits on is two subjects to count. **A RULE WAS PROMOTED TO THE HANDOFF §1: where a run acts on a measurement and records the action in the same field the measurement is computed from, the later measurement reports the action — check every rate's population for outcomes the run itself assigned.** **THE REAL-DATA SPIKE IS COMPLETE — first half 2026-09-07, second half 2026-09-10 — AND ITS FOUR READINGS ONLY MAKE SENSE TOGETHER: warm-starting saves 64.72% of cold iterations on real altimetry; 6.7% of that is the two-pass GEOMETRY and the rest is available from any converged start; ZERO of 289 points was re-ranked, on every arm, positive-controlled; and the selected candidate differs at 34% of points, ENTIRELY by candidates passing the conditioning gate in one arm and failing it in the other, of which 15 points are reachable from the exact optimum as the start.** A reader taking the first alone ships the two-pass architecture; the first two ship the cheap variant; the third calls warm-starting safe; **the fourth is the one needing a decision and it is not a decision about warm-starting — it is open question 23.** Verdicts: [first half](docs/superpowers/notes/realdata-spike-verdict.md), [second half](docs/superpowers/notes/realdata-spike2-verdict.md); sections [here](#the-real-data-spikes-first-half-is-done-2026-09-07) and [here](#the-real-data-spikes-second-half-is-done-2026-09-10). **PHASE 2d IS CLOSED 2026-09-06 (13 met / 1 reduced / 3 failed), its close is [here](#phase-2d-is-closed-2026-09-06--13-met--1-met-with-reduced-scope--3-failed-plus-2bs-two-inherited-failures-and-2cs-one), and TASKS 6 AND 7 STAY DECIDED AGAINST.** **Nothing from any verdict is restated in this head.**
...
7. **THE METHOD IS THE PRE-FLIGHT AND IT LIVES IN EXACTLY ONE PLACE:** [`phase1-to-phase2-handoff.md`](docs/superpowers/notes/phase1-to-phase2-handoff.md) §1 — (a0)–(a9), (a)–(k), the standing rules, the fixture facts, **and the ones 2c added:** (i2b) a high-ceiling control converts a null into a **located** null · (i2c) a sign-unstable benefit is worse than a small one · (j5) a second instrument is a cross-check only if it measures the same quantity under the same conditions · (j6) bound the unmeasured region before measuring it · (i11) refutation clauses in **both** directions · (a2b) make an invalid value **unavailable** rather than caveated · (h2) stratify only by axes at the metric's **own** granularity · (j7) never stratify by a quantity the treatment can move · **(h3) check every stratum boundary against the filters the population already passed — an eps-derived boundary and an eps-derived GATE arrive at the same number by the same argument, and the empty strata read as a finding about the data** · **(h4) a rule stated over "the metrics" must be checked against each KIND of metric — 2c hit this three times** · **(a2b) at a count: "zero cases" is a claim about the INSTRUMENT until proven otherwise** · (c4) a validator must be specified in the **coordinates and extent** the validated object actually has · **(c5) a gate over a set that can GROW must be written against the set, not an enumeration of its members** · **(c6) a practice enforced by a MECHANISM must be checked to reach every instance — a partially-installed guard prints a complete-looking green** · (e2) prove a mutant differs before recording a surviving mutation — **(e) now has six causes** · (e3) its opposite colour, a **red** suite hiding a dead assertion · (a2c) populated but nothing **acts** on it · (a2d) a hashed value's **unit** is part of its identity — **and 2d added its artifact register: when a measurement is too expensive to be a test, its recorded output is a current claim only while the configuration that produced it is, so the artifact carries its instrument block and the check FAILS when a named default has moved** · (a2e) encode a classification as a **construction** · (i12) a **uniform fixture set** cannot test a freedom the contract leaves open · (j8) an adopted verdict makes the **instrument** part of the specification · plus (a0)'s sixth register (**a check that never read the file prints the same word as one that did**) and (a4)'s two (**"checked" in your own pre-flight is a claim**, and **a decision's own EXAMPLE can be the case that refutes it — and a retired argument stays visible**), and (a5)'s across-DECISIONS register (**check a decision against the measurements taken for the OTHERS; a term of art repeated across decisions acquires a reading nobody chose**), **plus the four 2d added: (a5b) when two constraints bind the same quantity, solve them together and state which binds — a quantity sized against one and checked against neither is the common case, because each constraint is satisfied in the section where it is discussed · (j8)'s second register: a rate is a measurement of a WORKLOAD and the workload is a precondition, so a cost figure recorded without its fixture can be quoted and cannot be reproduced — and where a DETERMINISTIC proxy for a cost exists, budget in the proxy and convert late · (j7)'s budget-level cousin: a quantity the experiment can MOVE must be reported, not only used · **(j7b), promoted 2026-09-12: a rate over a population the run itself wrote into measures the run's DECISION — and a classification that has never been exercised has never been checked** · and (a4)'s agreeing-first-check rule gained the worked instance showing the error can point AT the expected answer — contention inflates seconds only, so a loud host pushed a per-iteration cost upward into a 0.3% "match" that is 19% apart when quiet**. **Do not restate them here** — the two copies drifted once already.

8. **CI IS VERIFIED BY ENUMERATING COMMITS, NOT BY READING `gh run list`** — a run between two green runs is not thereby green, and **a commit that is not the TIP of its push gets no run of its own**, because one push produces one run. **Enumerate by `headSha`, never by position**: a poll on *"is the first row completed"* returned the PREVIOUS run's row in this session and reported a queued commit as verified. **AS OF 2026-09-11, EVERY `src`-TOUCHING COMMIT IN 2d AND IN THE REAL-DATA SPIKE HAS A COMPLETED RUN WITH CONCLUSION `success` — and the spike added NO `src`-touching commit at all: all eight are docs, notes and harnesses.** **FOUR commits that touch no `src` have no run of their own, and the reason is the same every time: the next push cancelled the run verifying them.** **THE RULE THAT PREVENTS IT IS ONE PUSH PER RUN AND IT HAS NOW BEEN BROKEN THREE TIMES, THE THIRD ON 2026-09-11 BY COMMITTING THE SPIKE'S CLOSING RECORDS FOUR MINUTES AFTER ITS DECOMPOSITION** — so `8c4ee2d`'s run is `cancelled` and **a cancelled run is evidence of nothing.** What covers it is that the tip's green run tested a tree containing its content, which is weaker than a run of its own and is stated as such. **The mechanism is not carelessness about CI, it is that the post-commit hook publishes instantly, so "finish the thought, then commit" and "one push per run" pull against each other — and the cheap discipline is to WAIT FOR GREEN before the next commit whenever the previous one is still in flight.** (**Corrected 2026-09-03 from "docs-only"**: one of the two is a `feat:` commit that adds a harness under `notes/`, so the word and the commit type disagreed while the substantive claim — nothing in `src` is unverified — held either way.) A cancelled run is **evidence of nothing**; what covers those commits is that a later commit's green run tested a **tree containing their content**, which is weaker than a run of their own and is stated as such. **The rule that prevents it is one push per run**, and it was broken twice by committing again within minutes. **AND THE OPERATIONAL FORM IS AN UNTIL-LOOP ON THE RUN'S OWN STATUS, PROMOTED 2026-09-11 ON THE THIRD CAUSE:** `gh run watch --exit-status` returned **0** on a run that was still `in_progress` — it returned early, and **a wrapper that returns early and a wrapper that returns success are indistinguishable from the outside.** Same rule as the `&&` chain and the `| tail` pipe, third cause: *where a tool reports on something else, its exit code describes the tool.* So: `until [ "$(gh run view <id> --json status --jq .status)" = completed ]; do sleep 60; done`, then read `conclusion`. The general form is in [the handoff](docs/superpowers/notes/phase1-to-phase2-handoff.md) §2 with all three instances and is not restated here. See [THE CI FIXTURE DECISION](#the-ci-fixture-decision--taken-measured-and-verified-2026-08-22); **CI runs `-m "not machine"` and therefore executes exactly ONE of the nine RSS assertions**, so it is not a substitute for the local sweep.

9. **THREE DEFECTS ARE OPEN AND UNOWNED, AND NONE IS 2d's TO FIX.** (a) **`tiling.py` requires the spatial dims to be literally `y` and `x` in THREE FUNCTIONS — `read_amplification` (904–905), `assembly_spans` (932–933) and `assemble_tile` (974–975), SIX OCCURRENCES IN ALL** while stage 4a requires only that `time` is first. **CORRECTED 2026-09-12 FROM "four places", AND THE CORRECTION IS THE SECOND OF ITS KIND: `realdata-spike-preflight.md` already made it on 2026-09-07 and FIVE OTHER SITES STILL SAID FOUR five days later.** A correction recorded only where it was found is not a correction, it is a second version of the claim — **cite this defect by FUNCTION, never by line**, because the by-line citation has now drifted the record twice and the by-function one never has — so a `latitude`/`longitude` input **passes the contract and dies in assembly without exit code 4**, and **stage 4a's own message already calls the contract positional**, so the message and the implementation disagree today. **MEASURED 2026-09-07, TWICE, ON A SYNTHETIC `latitude`/`longitude` FIXTURE AND ON A REAL DUACS STORE: stage 4a passes, `read_amplification` raises `KeyError: 'y'` **in `read_amplification`'s `by_dim` lookup** (`tiling.py:907` as measured; **cite the function, the line is the part that drifts**) BEFORE the store is created, ~~`__main__` catches only `(ValidationError, InputContractError)`, and the process EXITS 1~~ — **THE EXIT-CODE HALF IS CLOSED, 2e TASK 1, 2026-09-12: the process now EXITS 5 (`INTERNAL_ERROR`), with its traceback, and 1 is unreachable by construction until 2e's Task 6 wires the failure-rate threshold.** ~~**THE NAME-DEPENDENCE HALF IS STILL OPEN and is 2e's Task 2.**~~ **CLOSED 2026-09-13 AT 2e's TASK 2 — DEFECT (a) IS FULLY CLOSED.** The tiling path addresses its spatial axes by position, a `latitude`/`longitude` store runs to exit 0, and the run produces **the same fits, array for array**, as the `y`/`x` store it was renamed from. **The closer taken was POSITIONAL-THROUGHOUT and the deciding argument was the FINGERPRINT**, not the site count: `geometry_components` keys `spatial_coordinates` by the input's own names and runs downstream of `open_input`, so a rename at the adapter would have made the store **assert a provenance its source file does not have**. **TWO THINGS CAME WITH IT, BOTH MEASURED, NEITHER IN THE BRIEF:** stage 4a now refuses **two spatial dimensions sharing a name** — xarray permits duplicates, such a store writes and reopens intact, and under positional tiling the y-slice was **silently dropped** (`(12,3,3)` asked for `(12,2,1)` returned `(12,1,1)`), turning a loud crash into a plausible number; and `store.py` now writes its coordinate arrays **under the store's own axis names, mapped by position**, because a lat/lon run previously produced a store opening as **five dimensions** in which `outcome(y,x,m)` had no coordinates at all. **Both were LATENT and UNREACHABLE, not new** — a schema that has never been exercised has never been checked. The struck text is kept because it is what made the ordering: this crash was the **live positive control** the catch-all was verified against, available only until Task 2 closes it — so Task 1 precedes Task 2 by (i2), and the guard that survives carries dated provenance. ~~A resuming script branching on 1 would resume from a crash that left nothing to resume.~~ **No longer reachable: a crash and a thresholded run are now different codes.** Two closers, both scope decisions — see [What plan Task 2 established](#what-plan-task-2-established-done-2026-08-24--read-before-touching-the-decimation-the-two-stores-identities-or-any-spatial-dimension-name). (b) **Open question 20**: what else is uniform across all sixteen input fixtures and unconstrained by the contract — **coordinate monotonic direction first**, because a decreasing latitude axis is the ordinary case in real altimetry and yields a *plausible* answer rather than an error. **2d WAS OFFERED THIS AND DECLINED IT, 2026-08-30** — grid shape and coordinate direction are different degrees of freedom, and closing half of it while (a)'s scope decision is open would give a suite covering a case the code still gets wrong. The reasoning is at the question. (c) **A FLAKY TEST, found 2026-08-30**: `test_completion.py::test_a_preempted_command_exits_aborted_early_and_resumes` polls **120 s** for the child to create a store, then signals regardless; on a 3.12 runner taking **33m21s** where 3.13 took **18m29s** the deadline expired and it surfaced as `GroupNotFoundError` — **a store-layer error for a scheduling failure**, (i9) across runners, and the presentation sends the next reader at zarr. Repair: record WHY the loop exited and fail with that.

---


## THE REAL-DATA SPIKE'S FIRST HALF IS DONE (2026-09-07)

> **Real altimetry fits are HARDER than every fixture this project has measured, so the warm-start
...
**THE VERDICT IS [`realdata-spike-verdict.md`](docs/superpowers/notes/realdata-spike-verdict.md)
AND NOTHING FROM IT IS RESTATED HERE.** The pre-flight, the predictions committed before the run,
the fetch instrument with its provenance, the harness, the report and the smoke record are the six
files beside it, all named in the verdict's head. **The second half is NOT begun and does not begin
without a check-in.**

What a cold session cannot re-derive, and which is therefore here rather than only in the verdict:

- **THE FIXTURE IS FETCHABLE WITH NO CREDENTIALS AND NO DEPENDENCY.** CMEMS `SEALEVEL_GLO_PHY_L4_MY_008_047`'s
  monthly ARCO store answers `.zmetadata` **200 anonymously**; only bucket *listing* is 403, so the
  zarr v2 key layout is **computed from the store's own consolidated metadata**. `fsspec`'s HTTP
  filesystem would have needed `aiohttp`, and **`pixi add` re-solves the lock every committed
  anchor was measured under** — the lock 2d's field digests are pinned to, with a documented
  failure mode saying exactly that. Mirroring over `urllib` costs forty lines and is why the
  control's equality below was still assertable.
- **THE CONTROL REPRODUCED THE 2d ANCHOR TO ZERO DIFFERENCE**, 384 points, two days later. That is
  available to any later session for under three hours and is the cheapest possible proof that its
  instrument and its environment are the ones these numbers came from. **It is an equality rather
  than a band because iterations are deterministic and `src/` had not moved** — check
  `git log <anchor-commit>..HEAD -- src/` before relying on it again.
- **THE SPIKE DID NOT FIX THE INPUT PATH.** It fitted a **renamed copy** and the un-renamed store's
  failure is a reading, committed before the rename existed. Both closers stay scope decisions and
  the defect stays unowned — see head item 9(a), which now carries its measurement.
- **`unique_dt = 6` ON THE REAL AXIS**, through the shipped contract. The handoff predicted the
  number from the calendar; this is the first real time axis this project has fed through stage 4a,
  and it makes a fixture fact a measurement.
- **THE `known_bias` NOTE INSIDE PREDICTION P6 WAS WRONG IN DIRECTION AND IS RECORDED AS WRONG.**
  It argued a six-timestep axis would push the real arms' seconds **high** against an `N`-scaling;
  they came in **22% low**, and the gap is left unattributed rather than given a story.
- **THE CONDITIONING FINDING BELONGS TO THE AUDIT, NOT TO THE SPIKE.** (h3)'s conclusion stands —
  `HESSIAN_COND_LIMIT` **is** `2²⁶`, so D9's upper `κ` bins remain unreachable — but the population
  the coincidence filters away is **a hundredfold larger on real data**, and §11.2's audit compares
...
>
> **THE SESSION'S RESULT, IN ONE SENTENCE:** *the pooled selection-disagreement rate decomposes as
> **all moves** on simulated data and **all dropouts** on real, so the pooled number describes the
> two populations identically and describes neither.* **That is why the decomposition had to
> arrive as a MEASUREMENT rather than as a definition** — the argument for separating the two
> quantities was available from D9 all along, and the population that shows the separation is
> real was not.
...
1. **THE RE-RUN WOULD CLOSE THE CRITERION ON THE WRONG POPULATION.** The rung field carries **1
   ...
   > **AND THE SMOKE TURNED THAT JUDGEMENT INTO ARITHMETIC, WHICH IS THE FORM IT SHOULD BE QUOTED
   > IN:** *52 points across 9 strata produce nothing quotable, and 384 across the same 9 would not
   > have either.* On the clean branch **every** point stratum is below the 30-member floor — the
   > largest holds 22 — so all three selection headlines are withheld. **That is the 10.18 hours
   > saved, stated as arithmetic rather than as judgement.**
...
**ITS PRE-FLIGHT IS [`wiring-one-preflight.md`](docs/superpowers/notes/wiring-one-preflight.md),
WRITTEN 2026-09-11 BEFORE ANY CODE, AND NOTHING FROM IT IS RESTATED HERE.** This section is the
brief; that file is the audit of it, and **the brief did not survive the audit intact** — see the
correction under addition 2 below, and W1 and W7 there for the other two.
...
2. **`κ` NOW HAS LIVE STRATA AND THEY ARE START-DEPENDENT, SO (j7)'s COLD-ARM BINDING IS DOING REAL
   ...
   > **CORRECTED 2026-09-11 AT THE PRE-FLIGHT, AND THE ARGUMENT ABOVE STAYS VISIBLE BECAUSE IT IS
   > THE ONE THAT WAS RETIRED.** *"`κ` now has live strata"* disagrees with (h3), with 2c exit
   > criterion 11's scope and with open question 23's own closing note, which all three agree:
   > `HESSIAN_COND_LIMIT` **is** D9's first boundary, so a cell above it leaves the both-OK
   > intersection and the upper bins stay unreachable. **Two arms moving their median `κ` does not
   > make a second bin reachable — it moves cells out of the population.** The check is still owed
   > and it is a DIFFERENT check: (j7) at the POPULATION, not at a stratum, read off the
   > intersection size that `CandidateOutcomes` already emits. **A second reported binning by the
   > warm arm is refused** — it would condition on a post-treatment variable to test whether
   > conditioning on a post-treatment variable matters. W3 of the pre-flight carries the
   > resolution.
...
## THE REAL-DATA SPIKE'S SECOND HALF IS DONE (2026-09-10)

> **Warm-starting on real altimetry saves 64.72% of cold iterations, more than 2c's 42.28% — but
> PROXIMITY is worth 4.36 points of it against 2c's 12.00, so the expensive geometry buys 6.7% of
> the benefit. The selected candidate differs at 34% of points and NOT ONE of those is a
> re-ranking: every one is a candidate that passed the conditioning gate in one arm and failed it
> in the other.**

**THE VERDICT IS [`realdata-spike2-verdict.md`](docs/superpowers/notes/realdata-spike2-verdict.md)
AND NOTHING FROM IT IS RESTATED HERE**, including its eight scored predictions. The pre-flight,
the predictions committed before the run, the harness, the report, the smoke record, the
decomposition addendum and its own report are the seven files beside it.

What a cold session cannot re-derive:

- **THE BRIEF'S 99.48% WAS A WITHDRAWN NUMBER AND THE PRIOR IS TEN POINTS LOWER.** That figure is
  the **self** arm against cold over points, corrected in this file on the day it was written. **2d
  has no point-level warm-vs-cold agreement figure at all.** The comparable prior is 2c's **90.37%
  (122 of 135), measured against a pre-agreed 90% stop that 121 would have failed** — so the
  mechanism was authorized on a simulated field with a margin of one grid cell.
- **THE SHIPPED TWO-PASS DRIVER AND AN IN-PROCESS WARM ARM RETURN THE SAME INTEGER.** 5877 against
  5877, relative difference 0.0, across a difference of driver, tiling and store round-trip. **So
  (j5) does not bite between those two paths** and a warm number from either is comparable to one
  from the other. Nobody had run it; it is the cheapest licence in the record.
- **THE CONTROL IS AVAILABLE AGAIN AND IT IS THE WARM PATH.** 2d's easy rung through
  `run_two_pass` reproduces `warm_per_point = 24.463541666666668` and `warm_ok_total = 9394` to zero
  difference in about 1.2 h, and the four-arm real measurement reproduced **all four totals
  exactly** on a second run a day later. **Check `git log f409b7f..HEAD -- src/` before relying on
  either.**
- **THE GEOMETRY WAS CHOSEN BY MEASUREMENT AGAINST 2c's INSTRUMENT, NOT BY ARGUMENT.** 17 × 17 at
  the shipped stride gives a warm radius mean of 2.602 against 2c's 2.556; the first half's
  1°-spaced grid would have put coarse points **8° apart against 1° in production**, understating
  proximity in the direction that would have condemned the machinery.
- **THE GEOMETRIC RADIUS IS NOT THE RADIUS.** With real coarse fits one cell of 27 fails and the
  spiral searches past it — mean 2.796, **max 8, beyond `DISTANT_MIN = 6`** — so on 9 cells the
  warm and distant arms are not separated at all. Reported both ways rather than repaired by moving
  a constant with the answer in view.
- **2c's OWN DISCRIMINATING CONTROL HAS A SILENT FALLBACK**, and it is (a0) at an arm: where no
  coarse point is far enough its harness takes the **warm** source, pulling `random` toward `warm`
  and shrinking the very quantity it measures. It cannot fire on this geometry and the harness
  asserts that. **Whether it fired in 2c's own run is not recoverable from the committed JSONL** —
  the direction is known and the magnitude is not, and the direction understates 2c's 12.00.
- **`DEGENERATE_HESSIAN` IS START-DEPENDENT, AND THAT IS A DEFECT WITH NO OWNER.** Median `κ` moves
  2.53e6 → 2.90e6 → 3.52e6 → 2.59e6 across cold / warm / random / self and the count above
  `HESSIAN_COND_LIMIT` moves 48 → 37 → 46 → 71. **The arm that does the least work has the most
  degenerate cells:** `self` converges in a sixth of cold's iterations and is refused 68% more
  often, gaining no `OK` cell and losing 54. **So any mechanism that shortens the path to the
  optimum moves which models are selectable** — a better initializer or a looser tolerance would do
  it too. Why the condition number itself moves is **not established**.
- **A ZERO WAS POSITIVE-CONTROLLED BEFORE IT WAS BELIEVED.** `MOVE = 0` is the comfortable reading,
  so the decomposition rule is run against a fabricated pair in which a move exists by
  construction, **before the quiet gate, on every invocation**, and the harness refuses if it does
  not find it.
...
### A SWEEP FAILURE WHOSE CAUSE WAS NOT ESTABLISHED, BECAUSE THE OUTPUT WAS PIPED (2026-09-11)

**`test_the_floor_ladder_reproduces_the_recorded_rungs` FAILED IN A FULL SWEEP AND THE ASSERTION
TEXT DOES NOT EXIST**, because the sweep was run as `pixi run test 2>&1 | tail -12`. **The standing
rule against exactly this is already in the record** — *never pipe a measurement's output through
anything*, and *local sweeps retain their full output* — written after a watermark failure that
could not be diagnosed. **It was broken anyway, in the same session that promoted the rule that
covers the other half of it.**

**WHAT IS ESTABLISHED, AND IT IS NOT A DIAGNOSIS:**

- **All five ladder rungs printed in that sweep's own summary were INSIDE their ±25% bands** —
  73.9 / 162.8 / 171.6 / 214.3 / 216.7 against 74.0 / 163.0 / 171.2 / 214.4 / 216.9 — and every
  relation the test asserts holds on those numbers. **So the failure was not a rung drifting**, and
  which assertion fired is unknown.
- **The test passed alone in 32 s at stall 0.0 ms/s**, and a second full sweep passed 1413/1413.
- **The failing sweep had four RSS readings above the 25 ms/s stall diagnostic, one at 774 ms/s**,
  against 0.0–4.5 ms/s on the sweep two hours earlier. The host was squeezed: load 4.73, 173 MB
  free of 15.8 GB. **The passing re-run was also squeezed** — four HIGH readings, up to 207.5 —
  which is why "the host was loud" does not finish the argument.

> **IT IS NOT FILED AS A SIXTH AMBIENT-CONDITIONAL FAILURE, AND THAT IS THE POINT.** *"Passed on a
> re-run under different conditions"* is a weaker claim than a diagnosis, and the evidence that
> would have told the two apart is the evidence that was destroyed. **Recording it as ambient would
> be assigning a cause to fit the convenient reading** — (a0)'s fifth register, tidy and
> retiring. **The honest entry is that a sweep went red once and nobody can say why.**
>
> **THE REPAIR IS THE RULE THAT ALREADY EXISTED**, applied: `pixi run test > <file> 2>&1`, never a
> pipe. The redirect also makes the exit code the command's rather than the pipe's, which is the
> other half of the same defect and is the rule promoted this same day.
...
| **Phase 2e implementation plan** | [`docs/superpowers/plans/2026-09-12-metamer-phase2e.md`](docs/superpowers/plans/2026-09-12-metamer-phase2e.md) — 8 tasks, 18 exit criteria **each naming its reading**, **APPROVED 2026-09-12; Tasks 0–6 DONE, Task 7 next** (the head above is the single source for this status). Implements design doc **§14.1 and §14.3 only** — §14.2 is 2f's, and the split is §17's measure/print rule applied. Carries fifteen decisions with their reasons, three corrections it owes to the tree and to this file, and the one-pass abort gap as a decision the sub-phase must take. Task 1 precedes Task 2 by (i2) and that ordering is not negotiable by convenience |
...
| # | question | closer |
|---|---|---|
| **20** | What else is uniform across all sixteen input fixtures and unconstrained by the contract? | **coordinate monotonic direction first** — a decreasing latitude axis is the ordinary case in real altimetry and yields a *plausible* answer rather than an error. The durable fix is the **first differently-shaped fixture**, not the corrected call site. **A SECOND ANSWER, FOUND 2026-09-12 WHILE CLOSING A DIFFERENT DEFECT: DUPLICATE DIMENSION NAMES.** xarray permits them and stage 4a checked only `ndim == 3` and `dims[0] == "time"`, so `("time", "x", "x")` passed the contract. Under name-based tiling it crashed loudly; under 2e Task 2's **positional** tiling a name-keyed dict collapses three dims to two keys and the y-slice is **silently dropped** — measured, `(12, 3, 3)` asked for `(12, 2, 1)` returned `(12, 1, 1)`. (**Corrected the same day**: the first measurement used `(2, 3, 4)`, which **cannot be stored** — `xr.Dataset` refuses inconsistent sizes for one dimension name — so it reported a size-collapse symptom that no reachable fixture can show. The mechanism and the hazard are unchanged; **the reachable witness has EQUAL extents**, and such a store writes and reopens intact.) **Closed by 2e's Task 2 with a stage-4a refusal**, in the task that created the hazard. **The instructive part is how it was found: not by sweeping for uniformities, but by trying to change one.** A uniformity is invisible until something depends on it differently |
...
dims to be literally `y` and `x` in ~~**three functions — six occurrences in all**~~ (~~four
places~~, corrected 2026-09-12; **CLOSED 2026-09-13 at 2e's Task 2 — zero sites, the path is
positional**) while stage 4a requires only that `time` is
...
the implementation disagree today.~~ **BOTH HALVES CLOSED 2026-09-13 at 2e's Task 2: such an input
runs to exit 0, and stage 4a's message is now TRUE rather than merely well-intentioned.** ~~Two
closers, both scope decisions.~~ **The positional one was taken, on the fingerprint argument.** **It is named at the close
...
> THAN 2c.** ~~**Three functions, six occurrences**~~ **— CLOSED 2026-09-13 at 2e's Task 2; the
> enumeration below is what it WAS.** `read_amplification`'s span tuples at
> 904–905, `assembly_spans`' `by_dim["y"]`/`["x"]` lookups at 932–933, and `assemble_tile`'s own
> `isel` at 974–975. (~~Four sites… `isel` at 973~~, corrected 2026-09-12: the count disagreed
> with its own list of three, and the `isel` line was cited one high.) **An input named otherwise passes stage 4a
...
> goes positional** (three functions, six occurrences, plus fixtures — and `tiling.py` is what was
> wrong). **THE SECOND WAS TAKEN, 2026-09-13 at 2e's Task 2**, on the fingerprint argument rather
> than on cost. What Task 2
> controlled was **not becoming ANOTHER site** (*"the fifth"* as written 2026-08-24, when the
> count was believed to be four). **A positional decimation above a name-based
...
**`batch/audit.py` builds four arms over one batch at one call site.** The four-reading table is in
its module docstring, **registered before any arm produced a number.** Two extractions landed in
`core/`: `optimize.ladder_start` and `fit.warm_start_faults`, each because the alternative was a
second spelling of a rule that has to stay bit-identical.

#### THE SUBJECT MOVED, AND THREE RECORDED DECISIONS MOVED WITH IT

The audit draws **fine** points and computes its own cold arm. The argument is at
[D6](#d6--the-coarse-stride-is-k--8-chosen-on-a-written-objective-and-closed-by-a-bound) and at
D10, and **not restated here.** One line: a coarse point sources itself, so its `warm` arm starts
from its own cold optimum.

#### THE SEED IS `audit.seed`, AND THE FIELD ALREADY CALLED `seed` IS THE TRAP

`Config.seed` is *"for anything stochastic"*, has **no consumer anywhere in `src`** — `fit` has no
stochastic component, which is D7's own reason a cold-versus-cold arm cannot exist — and is in
`FIT_RELEVANT_FIELDS`. **Using it would make re-running an audit at a different seed invalidate the
store it audits**, which is the boundary the audit block exists on the other side of.

**AND THE TEST THAT ENFORCES THAT BOUNDARY ENUMERATED TWO FIELDS.**
`test_the_audit_settings_move_no_gate` was written as `subsample = 500` and `stratify = true`, so
`seed` would have been added with the boundary **silently not covering it** — (c6), one day after
promoting it. It now derives the list from `Audit.model_fields` and **raises on a type it cannot
perturb rather than skipping the field**, because a skipped field is the same defect inside the
repair.

#### THE DIRECTION IS KEYED, AND THAT IS THE (k) FINDING

N2 introduces the **only randomness in the system**, so it is the only place §11.3's
traversal-independence can now be lost. **One `Generator` consumed in a loop loses it** — cell `n`'s
direction depends on how many were drawn before it, hence on the point set's order, hence on tiling
— **and recording the seed does not save that.** The direction is a pure function of
`(seed, GRID-GLOBAL point index, candidate)` through a `SeedSequence`. **The grid-global index, not
the row number**: keyed on the row, enlarging the subsample moves every existing cell's direction
and two audits of one store stop being comparable.

#### N1 AND N2 SHARE THE DIRECTION, AND `N1_EPSILON` IS DERIVED

The table's second row reads *"the sensitivity is to start DISTANCE, not direction"*, which follows
**only** if the two arms differ in nothing but distance. The three arms are a **magnitude ladder
along one ray**.

`N1_EPSILON = fd_step(1.0) = eps^(1/3) = 6.055e-06`, in unconstrained coordinates — **the step
`optimize_series`' own finite-difference gradient actually takes**, since it differentiates with
`curvature=None` and that ratio is one at every scale. So *"N1 non-zero"* means the answer moved
under a displacement **the optimizer cannot distinguish from zero**. A picked `1e-3` would make
that row read *"the surface has structure at 1e-3"*, which is an ordinary property of a likelihood.

#### AN INADMISSIBLE N2 START IS EXCLUDED AND COUNTED, NEVER RUN COLD

`r` is a real distance and the diagnostic box is finite, so an unlucky direction puts a start
outside it — and `_check_warm_starts` refuses the **whole call**. **The one-line fix is the defect**:
setting `x0_valid` false falls back to the moment ladder, so the N2 arm would silently contain
**cold** fits and *"N2 agrees with cold"* would be true by construction at exactly the cells where
the perturbation was largest — (a0)'s fourth register. One admissibility array is built and both
`x0_valid` and the result mask derive from it.

Two more classes are counted rather than folded in, because each **looks like agreement**: `r = 0`
(N2 collapses onto cold) and an exhausted spiral (no warm start, so no distance, and no hysteresis
question either).

#### SEVEN MUTANTS: SIX CAUGHT, AND THE SEVENTH PROVEN EQUIVALENT RATHER THAN LISTED

Caught: N1 given a fixed direction, N2 matched on the **mean** distance, a streamed RNG, a
direction drawn at `p_max` for every candidate, a seed recorded but unused, and the warm arm run
cold.

**THE SURVIVOR IS `cold_starts`' `objective` ARGUMENT, AND IT IS NOT A COVERAGE GAP.** Substituting
`REML` for `ML` there changes nothing: the ladder start is `moment_init` — which takes the **spec**
and the data, neither an engine nor an objective — followed by `to_unconstrained`, a per-parameter
transform. **Measured: `max |difference| = 0.0`.** (e2) satisfied by proving the mutant does not
differ rather than by recording a surviving mutation. **The argument is passed anyway**, because
what is wanted is the objective the audited fit ran under, and a parameterisation that ever became
objective-dependent would otherwise make the floor arms perturb around a point production never
started from. Recorded at the function.

#### A FIXTURE FACT THAT COST TWO FAILING TESTS

**A hand-built decimal-year axis is not `to_decimal_years`.** `2000 + i * 31/365.25` over the same
monthly coordinate moved `θ̂` by **6.7e-05 relative** — large enough to fail a bitwise comparison
against the store, small enough to read as a floating-point detail and be argued away. **The
conversion is under `ALGORITHM_VERSION`**, so a second derivation of it is a second derivation of
fit identity. Every test here takes the axis from the input.

#### AND THE SUITE'S COST WAS CUT BY 4×, WHICH IS RECORDED BECAUSE IT CHANGES NOTHING ELSE

`tests/test_audit.py` ran at **495 s** with three end-to-end tests each building their own two-pass
store and their own arms. Module-scoped fixtures for the store and for the two arm sets, and an
audited point set capped at **8** fine points rather than all 31, bring it to **121 s**. Every
assertion is per cell over whatever cells exist, and the tests that need a specific cell
**construct** it, so nothing depended on the count.
...
> ## AND A SECOND CONDITION ON EVERY 2d FIGURE, MEASURED 2026-08-30 AT TASK 1
>
> **2d's FIELD IS EASIER THAN THE ONE D1's 42.28% CAME FROM, AND EVERY 2d NUMBER CARRIES THAT.**
> After the family-change rebuild it runs at **24.4 cold iterations per point** at `N = 630`,
> against 2c's **40.79** — level with Task 0's plain smooth batch and **3.6% below the band this
> was predicted into.** **The saving is bounded above by what the cold start leaves on the
> table**, so **any saving 2d reports is a saving at LOWER DIFFICULTY** and is not comparable to
> 2c's headline without that qualifier. **This conditions Tasks 5 through 7's numbers**, which is
> why it sits here rather than only in Task 1's record.
...
>
> **TWO CONSTRAINTS ON THE MIDDLE RUNG, BECAUSE IT OCCUPIES THE SLOT THAT USED TO BE
> PLAUSIBILITY.**
>
> 1. **IT IS OURS TO CHOOSE AND MUST SAY SO** — in `Rung.sources`, and **in every figure drawn
>    from it.** A middle rung on a sweep is **exactly where a later reader supplies "plausible"
>    for free**, and the slot's history makes that likelier rather than less.
> 2. **ITS `ℓ` AND `Δ` SIT BETWEEN THE TWO SHIPPED RUNGS ON THE SAME DIAGONAL**, so the three are
>    **one lever's curve** rather than three fields. **If it drifts off the line the sweep stops
>    being a sweep** and becomes three points with no ordering — and E6's monotonicity prediction
>    would have nothing to be monotone in.
...
> | **WHAT 2d ACTUALLY BUYS — re-priced on MEASURED seconds, 2026-09-01** | see below | **~25 h** |
>
> ## 2d IS NO LONGER THREE ARM-SWEEPS. IT IS ONE NULL, ONE DIFFICULTY RUNG, AND A LADDER OF ITERATION COUNTS
>
> **Decided 2026-09-01 on Task 5's result.** Tasks 6 and 7 as planned would spend ~20 h moving
> `Δ` — which Task 5b's pre-flight then measured to be **mostly an amplitude the likelihood is
> invariant to** — while holding difficulty fixed at the value that produced the null. **Three
> nulls at one difficulty is one null reported three times.**
>
> | what 2d has or buys | cost | state |
> |---|---|---|
> | the three rungs' **cold iteration counts** — 24.375 / 24.333 / 24.396 | spent | **measured 2026-08-31.** It is what shows the contrast lever does not move difficulty |
> | the **easy rung's full arm sweep** — null clean, all arms at the floor, saving −2.45% | **10.18 h, spent** | **measured 2026-09-01** |
> | ~~the middle rung's arm sweep (Task 6)~~ | ~~10 h~~ | **NOT RUN.** Same difficulty, weaker contrast |
> | ~~the hard rung's arm sweep (Task 7)~~ | ~~10 h~~ | **NOT RUN**, same reason |
> | the **difficulty rung's calibration** — the lever's value chosen against a measurement | ~1 h | Task 5b |
> | the **difficulty rung** itself | **~13–17 h**, priced on measured seconds at the chosen difficulty | Task 5b |
>
> **THE RUNG'S PRICE RISES WITH ITS DIFFICULTY AND THAT IS THE BINDING CONSTRAINT.** At the easy
> rung's difficulty an arm costs **13.98 s/point**; at `white/sigma = 0.8` the probe measured
> **26.48 s/point**. **6.016 arms × 384 points** is therefore **10.2 h at 13.98 s and 17.0 h at
> 26.5 s**, so *"land near 2c's 40.79 iterations"* and *"fit the remaining ceiling"* are two
> constraints on one number — **(a5b), and they are solved together at the calibration rather than
...
### What plan Task 4 established (done 2026-08-31 — read before touching the report, the gate or the instrument block)

**THE MODULE DOCSTRING AND THE PRE-FLIGHT CARRY THE DESIGN; NEITHER IS RESTATED HERE.**

**THE GATE LIVES IN THE DRIVER, AND THE MECHANISM IS THE ORDERING RATHER THAN THE FLAG.** A
reading that exists can be read, so a report carrying a width beside `contaminated: true` has the
number in it and the number is what travels. **The null is taken first and on contamination the
widths are NEVER COMPUTED** — `build_report` refuses a contaminated rung that was handed widths,
because that combination can only come from a caller that computed them before checking. **(a2b)
at its strongest: not *made unavailable* but *never made*.** The across-rung half is
`require_clean`, which **raises**; `run_rung` still returns a contaminated report, because E6 says
*stop and diagnose* and the diagnosis is the null's own profile.

**A SMEAR READING HAS THREE STATES AND `Quantity` HAS TWO.** A measured width; **at the floor**,
which is a valid reading with no number; and **refused**, which is no reading at all. Folding the
last two together makes *"the instrument looked and resolved nothing"* the same bytes as *"the
instrument did not look"* — (a0)'s excluded-versus-missing register at a report field. **So the
report carries the `WidthReading` beside the `Quantity`**, and on a contaminated rung the reading
is `None` rather than refused, which is a third distinct fact.

**ITERATIONS ARE INSIDE `reproducible()` AND SECONDS ARE NOT**, on the strength of the third
rung's measurement rather than by convention: the same fixture reproduced its iteration counts to
every digit while its seconds moved 15%. That turns the byte-identity invariant from *"the numbers
that happen not to be timings"* into *"everything the run determines"*.

> ## BOTH BRANCHES OF THE GATE FIRED ON REAL RUNS, AND THE CONTAMINATED ONE PROVED TASK 2's PROFILE RULE
>
> **The capped smoke run's null returned 26.0 cells — the whole normal axis — and its profile was
> FLAT AT 1.0 at every index.** Not a band, not a slope. Task 2 named two causes for a firing null
> and said the profile separates them; **this is unambiguously the second — the baseline
> disagreement rate itself above a half — and `cells = 26.0` alone could not have told it from the
> first.** The profile earned its place on the first real run that used it.
>
> **And the cause was the recorded fixture fact.** At `max_iter = 8` the Matérn candidates never
> reach `OK`, so `white` wins everywhere and disagrees with both regimes; `coarse_ok` then finds
> no usable coarse fit, the spiral is exhausted at **all 52 points**, and the N2 map is empty.
> **One low iteration cap produced a degenerate selection axis, an empty floor arm and a firing
> null, and the three are the same fact.**

**AND ONE OBSERVATION THAT IS NOT A FINDING, FLAGGED BECAUSE IT WILL LOOK LIKE ONE.** On the clean
smoke run **cold's smear width is 13.0, not zero** — a cold arm has no warm start, so that width is
misclassification near the boundary and nothing else. **A smear read as warm's absolute width
would be reading 14.0 where 13.0 of it is not hysteresis.** That is the N2 arm's whole argument
arriving as numbers. **It is an observation on a 26 × 2 fixture at `n_time = 24`, where the
majority threshold is over two points; no magnitude transfers**, and Tasks 5–7 are what measure it.

**`build_field` GAINED `n_normal` / `n_parallel`, AND THE RISK IS MITIGATED BY THE BLOCK.** The
driver could not otherwise be exercised at all: 384 points at 11–13 s each, and **a field small
enough to be fast cannot carry the interior null** — the shipped offset is 12, so `n_normal ≥ 26`
before a legal null line exists. The instrument block records the geometry and `is_a_smoke_run`,
**in the block rather than in a filename, because a filename does not travel with the bytes.**

**(e2) FIRED FOR THE THIRD TIME, AGAIN ON MY OWN SWEEP.** Sixteen mutations, one survivor, and the
survivor was `rung if value is not None else rung` — **a no-op**. Re-run as a real mutant it was
killed. **Three instances now, all self-caught, and all three were mutations that did not mutate.**
...
**WHAT IT EXPLAINS — three findings collapse into one cause:** the easy rung's null, where a warm
start had nothing to improve because the same construction **with** a signal sits where 2c measured
its saving; **E5's diagonal** being invariant in a quantity the likelihood cannot see; and **a
difficulty no noise-floor setting could reach**, the missing distance never having been in the
noise floor at all. **Each of the three is paired with its measurement in the note, once.**

> ## WHAT IT INVALIDATES, AND THE DISTINCTION IS THE POINT
>
> **THE THREE-RUNG NULL IS NOT WITHDRAWN. ITS SCOPE IS NARROWED.** It is a measured result *on
> fields with no signal*, and **whether it transfers to a signal-bearing field is exactly what a
> rung tests.** Every number in the easy rung's report is still what it was measured to be; what
> moves is the population it describes.
>
> **Criterion 17's ladder is unaffected as a measurement** — the three fields cost what they cost —
> **and its interpretation moves**: three samples of one difficulty rather than a sweep. **The
> figures are in the note and in criterion 17's own record; they are not spelled here.**
...
concentrated likelihood is invariant to amplitude; `rho` in samples is worth about one iteration
across the ladder that was run. **So `Δ` moved a quantity the fit cannot see**, the three rungs'
identical difficulty is structural, E6's monotonicity had no lever to be monotone in, and E5's
crossed-rung follow-up is withdrawn. **The `ℓ` leg is untouched.** **The amendment and its two
measured pairs live at E5, once** — they were spelled twice here and one copy is deleted.

## 3. THE 2c PROBE, AND 2c's FIXTURE IS IN THE TREE

**Contrary to a claim this sub-phase repeated** — `fields.py`'s docstring said the 2c spike's field
*"lived in a script that is not in the tree"* — **it is at
`docs/superpowers/notes/warmstart-spike-harness.py`**, fully specified, and the probe imported it
rather than re-spelling it. What was true is that it was not in `src`, so criterion 12 could not be
re-measured **on the shipped mechanism**. **THE DOCSTRING IS CORRECTED, 2026-09-03**, in the
narrower form, with the reason the wrong form cost something recorded beside it.

**Four settings, 16 points each, `N = 630`, the shipped `fit`** — 2c as its harness builds it, 2c
with its signal removed, 2c on 2d's time axis, and 2d's own parameters. **The signal accounts for
almost the whole gap between the two constructions; everything else 2c differs in, combined,
accounts for about one iteration.** **The table is in the note and is not copied here.**

## 4. THE CALIBRATION LADDER — KEPT, BECAUSE IT IS THE MEASURED ALTERNATIVE

The ladder over `white/sigma` reports iterations per point **and** misclassification at each
setting; **`white/sigma = 1.0` was the setting the constraints chose, and separability rather than
cost is what ruled out the top rung.** **IT IS NOT THE ROUTE, DECIDED 2026-09-03** — the signal
reaches 2c's difficulty at **no** separability cost, and the ladder never reached it — but **it is
not deleted**: if the signal route fails, this is the measured fallback and its numbers are already
paid for. **Every figure is in
[`phase2d-difficulty-calibration-verdict.md`](docs/superpowers/notes/phase2d-difficulty-calibration-verdict.md),
once.**

## 5. THE SEPARABILITY CAUTION, AND 2d DOES NOT INHERIT IT

**2c's misclassification is where the interior null would be at risk** — at 2c's rate,
`P(Binom(12, p) ≥ 7)` is not small and the null reads **two** seed rows. **On 2d's parameters the
signal changes it by nothing, measured both ways** (the rate is in the note). **The signal costs
difficulty and not separability; 2c's rate is its noise floor's doing.** **Re-check this at any
raised noise floor**, because the two constraints couple there and not here.

## 6. AT ANY RAISED BASELINE THE PROFILE IS THE PRIMARY READING, NOT THE WIDTH

**A smear must push a row past 0.5 to register as a width at all.** At the baseline 2d's parameters
give, that is a large flip; at the ladder's mid settings it is a much smaller one — **the rates are
in the note and the calibration verdict.** **Task 2's recorded forfeit — the majority rule is blind
...
## 7. THE DECISION, TAKEN 2026-09-03 — FIXED FOR EVERY FIELD, AND THE THREE RUNGS ARE NOT RE-RUN

**(a) THE SIGNAL IS FIXED FOR EVERY FIELD. TREND ONLY, AT 16 `sigma` OF RISE OVER THE RECORD.**
A trend-estimation benchmark carrying no trend is the defect; **one carrying a trend on some fields
and not others is the same defect with a switch.** The signal is a property of the **builder**, and
**the rung's lever stays whatever the rung's lever is.** **No offset** — it is bit-identically
nothing, and adding it for symmetry would transcribe a constant nobody chose. **The unit is the
rise over the record in multiples of `sigma`**, never a bare coefficient, because a coefficient is
not portable across record lengths or noise levels. **The magnitudes, the linearity and the
separability readings are in
[`phase2d-signal-defect.md`](docs/superpowers/notes/phase2d-signal-defect.md), once.**

**(b) THE THREE SHIPPED RUNGS ARE NOT RE-RUN, AND THE PRICE IS NOT THE REASON.** They are not
re-run because **their result is already correct for what it measures**: a null on signal-free
fields, with the mechanism running to completion and changing nothing. **That does not improve by
being repeated on a different construction — it would become a different measurement wearing the
same name**, which is the comparability failure this project has hit repeatedly.

**WHAT REPLACES RE-RUNNING THEM IS ONE RUNG ON THE CORRECTED BUILDER.** So 2d reports: **three
rungs' null on signal-free fields, one rung on the corrected builder at 2c's difficulty, and the
contrast between them.** **The two constructions are the comparison rather than a confound, because
the difference between them is exactly the term whose absence was the defect.** If an artifact
appears at the new rung, that contrast is the finding. **If none appears there, the pre-decided
stop applies cleanly** — a null at 2c's own difficulty, on a field carrying what 2c's carried.
**One rung fits the remaining ceiling; the price is in the note.**

**(c) THE THREE ANSWERS THE PRE-FLIGHT ASKED FOR, TAKEN 2026-09-03.** **The trend's rise is scaled
by the CELL's sigma**, because the fit sees rise/sigma and not rise, so per-cell adds no structure
in any quantity the fit can see while a field-level scaling puts a difficulty step on the boundary
— and because per-cell keeps the field `dimensionless truth × BASE`, which is what the construction
already is. **The rung aims at 2c's difficulty AS THE SHIPPED `fit` MEASURES IT**, which is the one
condition under which the comparison is like-for-like; **the other reading, from 2c's own optimizer
path, is not deleted** — the two are different instruments on one fixture and the gap is a recorded
fact about them. **The marker is a FIELD-CONSTRUCTION VERSION, not a signal flag**, because the
signal is not the first thing to move the drawn bytes — the Cholesky change is a prior instance of
the same class, and **checking rather than assuming shows it predates every committed rung number,
so it is part of what version 1 already is.** **The reasoning and the checks are in
[the pre-flight](docs/superpowers/notes/phase2d-preflight.md), once.**

> ## THE CONSTRAINT THAT RIDES WITH IT, AND ITS GUARD NOW EXISTS
>
> **THE GUARD THE CONSTRAINT RESTED ON DID NOT EXIST, AND THE CITATION OF IT WAS MINE.** *"A test
> asserts it"* named a mechanism nobody had checked for: the suite compared `FieldTruth.parameters`,
> which a signal added after the draw leaves untouched, **so the cited guard would have printed
> green through exactly the change it was cited against.** (a0)'s register **at a citation rather
> than at a value**. **Written, run green on the unchanged builder and committed BEFORE `fields.py`
> moves** — a guard written after the change is a guard fitted to the outcome.
>
> **It compares what it claims to compare**: the stored field for each shipped rung, at the recorded
> seed and the shipped geometry, against a committed digest — **and, separately, that the field
...
> must not change.** If the repair moves their values, **the null's artifact is invalidated
> retroactively** — a recorded number about a field nobody can rebuild.
>
> **DRAWN BYTES AND FIELD VALUES ARE NOT THE SAME OBJECT.** A signal added as a term **after** the
> draw leaves the RNG stream untouched while moving every stored value, so **an assertion written
> over the draw would print green through exactly this change.** The pre-flight states which of the
> two the shipped test asserts **before** any code. **If field values move, the fix carries a
> version marker and the signal-free construction stays reachable.**

## 8. THE STATE OF THE TREE

**Every measurement in this handoff is committed with its predictions committed first**, and each
verdict names its own instrument. **The `src` edit owed at item 3 is made, 2026-09-03** — a
docstring correction, verified by the full sweep on the tree that ships. **The quiet gate refused
one run of the calibration at load 5.12 and no number from it exists**, which is Task 0's fourth
defect not repeating.

> **AND THE FOUR RECORD CORRECTIONS TAKEN WITH THE DECISION, 2026-09-03.** (1) **The plan's Task 5b
> named `white/sigma` as the lever and did not mention the defect at all** — which is what a cold
> session would have executed. Amended at its source, the noise-floor route **struck rather than
> deleted**, with the measurement that retired it. (2) **Task 8 depended on Task 6, which is decided
> against** — an unsatisfiable gate, and its behaviour named a *"plausibility rung"* that will not
> exist. **Re-pointed to Task 5b**, and the figure now carries the three-rung null beside the new
> rung's width, because the contrast is the finding. (3) **The signal probe's measurements were
> spelled twice**, in a section that said they were not; **one copy deleted, the note is the home,
> this section points.** The same rule applied to E5's two pairs, which were also spelled twice.
> (4) the docstring above. **No pair disagreed** — which is the condition under which the rule is
> cheap, and the reason to apply it then rather than after a drift.
...
**THE FIGURE IS ONE RUNG AND TWO CONSTRUCTIONS, NOT THREE RUNGS.** Only the easy rung ever ran the
full audit, so it is the only rung with arms and profiles — middle and hard contributed criterion
17's iteration counts and **no widths at all**. The plan's *"the three-rung null beside the version-2
null"* would have drawn two rungs' worth of widths that do not exist. **What is drawn is the same
rung, seed and geometry at both field constructions**, which is also the cleaner comparison: the
single difference between the panels is the term whose absence was the defect.

**THE DRAWN QUANTITY IS THE PROFILE, AND THE WIDTH IS AN ANNOTATION.** Every width is at the 1-cell
floor on every arm at both constructions — six readings of one censored value. **A chart of six
identical censored numbers shows nothing**, and the record already said the profile is the primary
reading wherever the baseline leaves zero. The majority threshold is drawn as the line a smear would
have to cross.

**RENDERING IT CAUGHT FOUR DEFECTS THE CODE COULD NOT, AND THAT IS THE TRANSFERABLE PART.** Titles
collided with subtitles; three direct labels stacked on one point because the arms peak at the same
row; **the cold arm was entirely invisible** beneath warm and N2; and the dumbbell's two dots
overlapped wherever the arms agree. **The repair for the third is the one worth keeping: line width
is the secondary encoding**, cold widest and underneath, so a reader sees blue wherever nothing
moved — which is nearly everywhere, and is the finding. **Look at the picture; the validator checks
colour, not layout.**

> **A TEST THAT DID NOT BITE, FOUND BY CHECKING THAT IT DID.** The first version of
> *"the figure is drawn from the committed reports"* compared the generator's extraction against a
> dict the test built **from the same file it had just loaded** — a tautology that **passed with a
> report deliberately corrupted underneath it**. The replacement writes a **provenance sidecar**
> naming each source report by **content hash** beside the arrays drawn, and the test recomputes
> those hashes from disk: **a report edited without redrawing the figure now fails.** (e2) says
> prove the mutant differs before recording a surviving mutation; this is its converse — **prove
> the test fails before recording it as a guard.**

**AND THE README SWEEP FOUND FOUR FALSE CLAIMS**, one of which is the finding: the two-pass section
said warm-starting *"cut iterations substantially"* on a simulated field, **which is true only of a
field carrying a trend and false on the construction the three rungs used.** The others: 2c
described as in progress, `bench` described as only the evaluation-path harness, and a pinned test
count that had aged by 175. **THE COUNT WAS REMOVED RATHER THAN UPDATED** — a number nothing
verifies ages silently, and a test that checked it would fail on every `-k` subset run, so the
README describes the suite and points here for the number. **Removing an unverifiable claim beats
maintaining it.**
...
> ## THIS IS THE ABSENCE OF HYSTERESIS, NOT A RESOLUTION LIMIT — AND THAT IS THE STRONGER RESULT
>
> **The interior null came back at the floor** — E6's third row held, as Task 2 committed before any
> rung ran. **Then all three arms returned the floor**, and the reason is not that a width sat under
> the estimator's threshold: **the warm and cold misclassification profiles are IDENTICAL at all 32
> indices.** Every point was warm-started, none was excluded, none had a zero displacement, and the
> selection axis was live. **The mechanism ran to completion and changed nothing.**
>
> **A marginal width would have been a weaker finding than this.** *"The instrument could not
> resolve it"* leaves the artifact's existence open; *"warm equals cold everywhere on a field where
> every point was warm-started"* does not. **The framing matters because the two send the next
> session to different places** — one to the estimator, one to the field.
>
...
**ONE FINDING WITH TWO LEGS, AND IT EXTENDS D1's REATTRIBUTION ALONG THE SAME AXIS.** The saving
came in at **−0.41% (pass 2 only) and −2.45% (net of pass 1)** on a field running at **24.4 cold
iterations per point**, below 2c's shortest fixture — which is what the reattribution's curve says.
**Its second leg is that the ARTIFACT follows the same axis:** at 24 iterations from a moment-ladder
start the optimizer reaches the same optimum from either start, so there is nothing to keep and
nothing to smear. **The saving is mostly *"start from any converged optimum"* and the artifact is
mostly *"the cold start was poor enough to be improved on"* — one statement about difficulty, not
two statements about coherence.** **The lever that produces hysteresis is the LIKELIHOOD's
difficulty; `Δ` only makes the boundary stark.**

> ## AND `Δ` IS MOSTLY AN AMPLITUDE RESCALE, WHICH THE LIKELIHOOD IS INVARIANT TO — MEASURED 2026-09-01
>
> `build_field` computes `parameters = factor × BASE`, so **one factor multiplies `sigma`, `rho`
> and `white` together** and `white/sigma` is **constant everywhere on the field by construction**.
> At `contrast = 3` region B is `(2.5, 2.0, 1.0)` against region A's `(1.0, 0.8, 0.4)`: the same
> series, 2.5× larger, with a 2.5× longer timescale.
>
> **Measured at Task 5b's pre-flight, 8 series at `N = 630`: the same data scaled by 2.5 fits in
> 23.50 iterations per point against 23.62 unscaled** — free, as a concentrated likelihood must be.
> **So the only part of `Δ` the fit can see is `rho` in samples, and that is worth about one
> iteration** (23.62 at 9.4 samples per `rho`, 24.62 at 23.6, 24.75 at 3.5).
>
> **THAT IS WHY THE THREE RUNGS ARE INDISTINGUISHABLE IN COLD ITERATIONS** — 24.375 / 24.333 /
> 24.396 — **and it is a property of the lever's construction rather than a coincidence.** The
> sweep holds difficulty fixed and moves an amplitude.
...
> ## CORRECTED 2026-09-01 BY TASK 5's MEASUREMENT: ITERATIONS ARE THE RIGHT UNIT FOR REPRODUCIBILITY AND THE WRONG ONE FOR MONEY
>
> **The conclusion above is right about determinism and was over-extended into pricing.** Task 5
> measured the two coming apart in **both** directions on one run:
>
> | arm or setting | iterations, against cold | seconds, against cold |
> |---|---|---|
> | the `self` arm | **0.0895** | **0.460** — 5.1× more seconds per iteration |
> | a noisier field (`white/sigma` 0.4 → 0.8) | **+22%** | **+93%** — 1.6× more seconds per iteration |
>
> **A fit pays a fixed per-cell cost — state space, ladder start, scoring, ranking — that does not
> shrink when the optimizer stops early**, and a harder likelihood costs more *per iteration* as
> well as more iterations. **So a budget built in iterations under-prices a fast arm by ~5× and a
> hard one by ~1.6×**, and Task 0's own *"`self` is 4–6% of cold"* would have mis-predicted its
> wall clock five-fold.
>
> **WHAT SURVIVES UNCHANGED:** iterations are what a committed artifact must record, what two runs
> are compared on, and what a re-measurement must reproduce — Task 5's cold arm reproduced
> criterion 17's **2340** exactly, a day and a code path apart. **What changes is that a COST
> estimate must be built from measured seconds per arm at the difficulty being priced**, and that
> is why E2's factor was re-priced twice from seconds and not once from iterations.
...
**WHAT A TASK-0-SHAPED SPIKE CARRIES FROM NOW ON**, one line, because all four defects were in
this list's absence: **dtype identical on both sides of any comparison · the time axis built by
`to_decimal_years` and never by hand · fixture identity held FIXED across repeats, so a spread
measures the machine and not the data · and the quiet check GATING rather than annotating.**
...
- **THE FLOOR LADDER THAT GOES TO THE SUMMARY IS THE ONE NOTHING ASSERTS, AND ON 2026-09-14 IT
  MOVED BY A THIRD AND WENT NON-MONOTONIC.** `test_memory.py` calls `measure_floor` **twice** in
  one sweep: *"the floor ladder's rungs"* checks every rung against a ±25% band, and *"the floor
  with the input open"* checks only **relations** and says so — *"a diagnostic, not an
  assertion"* — and **that second one is what appends the `floor ladder (MB)` line to the summary.**

  Three consecutive sweeps printed `xarray_zarr=163.2 / 163.0 / **122.4**` and
  `metamer_batch_run=172.0 / 172.0 / **116.8**`. The third is **below** `xarray_zarr`, which is
  impossible for a ladder read cumulatively — `_FLOOR_CHILD` imports in one child and calls
  `current_rss_bytes()` after each step, so the sequence can only fall if **pages were reclaimed
  between two reads.** That sweep reported **5 of 6** RSS measurements above the 25 ms/s stall
  diagnostic, which is the same host condition from the other side.

  **The asserted call passed in that sweep**, so the checked measurement was in band while the
  printed one was 32% low — `0.75 × 171.2 = 128.4` against `116.8`. **The two calls measure the
  same quantity in the same session and disagreed by ~50 MB.** Nothing is wrong with the code; what
  is worth not forgetting is that **the ladder in the summary is a reading nobody checks, taken
  under whatever pressure the host was under, and it must not be quoted as a floor.** Quote the
  banded one, or take it on a quiet host — the gate for that already exists (open question 22).
  `current_rss_bytes()` is CURRENT, not peak, and is the only rung reader that can go down.

  **CONFIRMED THE SAME DAY BY AN UNPLANNED CONTROL.** The sweep was re-run on the **identical
  tree** an hour later, for an unrelated reason (a test fixture had changed after the first run).
  The ladder came back **`73.9, 162.8, 171.6, 214.8, 216.1, 281.7` — monotonic, and within 0.5 MB
  of the two sweeps before it** — with **1** RSS measurement above the stall diagnostic rather
  than 5, and the sweep itself ran **1:07:05 against 1:40:14**. **Same code, same fixture, same
  box: the 50 MB and the inversion were the host and nothing else.** That is the positive control
  this reading would otherwise have lacked, and it is why the entry says *do not quote it* rather
  than *investigate it*.
...
- **TWO FIELDS THAT DIFFER ONLY IN THEIR VALUES SHARE A `fit_hash`, AND FOR A CONSTRUCTED FIXTURE
  ...
  says it at the top: the fingerprint is over **the type, not the values**, and *"a value edit at
  fixed geometry does not move it."* **This is correct for the production package** — data lives at
  a URI, and identity covers the geometry the fits were computed against. **It is wrong-shaped for a
  benchmark fixture, whose CONSTRUCTION is part of its identity and is hashed nowhere.** A
  signal-free and a signal-bearing field, at one rung and one geometry, are **the same fit identity
  and different data**. Consequences, and neither is 5b's alone: **nothing may use fit identity to
  tell two constructions apart**, and **any resume or `--reuse-fits-from` path keyed on it can cross
  them silently** — a warm start or a cached fit from one construction satisfying a run of the
  other, with no error anywhere. The instrument block is where a construction has to be recorded,
  and **it currently names the terms the config FITS rather than the terms the builder DRAWS**, so
  it does not distinguish them today either.
...
24. **IS `INSUFFICIENT_DATA` IN THE FAILURE-RATE DENOMINATOR? THREE SOURCES GIVE THREE ANSWERS,
    ...
    2f**, which owns §14.2's denominators.
    ...
    **THE FINDING IS THAT §8.6 IS STALE, NOT THAT `is_eligible` MAY NEED CHANGING.** §12.5
    separates land and permanent ice (`NOT_APPLICABLE`) from a genuinely thin record
    (`INSUFFICIENT_DATA`), states that *"collapsing them makes the failure rate
    uninterpretable"*, and corrects §14.1's wording in the same passage. **§8.6 still describes
    `INSUFFICIENT_DATA` as "too few valid samples — land, permanent ice", which is the exact
    conflation §12.5 was written to undo.** §8.6 carries a dated superseded-note as of
    2026-09-14, because **a reader looking for denominators reaches §8.6 first** and would take
    it as authoritative — (a6): a description whose subject has moved is struck where it sits,
    not only corrected where its successor lives.

    **WHAT IT WOULD COST, WHICH 2f NEEDS IN ORDER TO SCOPE IT: this is a re-baselining, not a
    classification edit.** `INSUFFICIENT_DATA` **has producers and occurs in real data**, so
    moving it into the denominator changes **every failure rate this project computes**, and any
    committed artifact carrying a rate computed under the current rule becomes **incomparable**
    to one computed after. **2f's first act on this question is to check whether such an
    artifact exists** — exactly as 2e's Task 3 checked and found the empty set.

    **WHY IT WAS NOT BUNDLED INTO TASK 3, RECORDED BECAUSE THE PRINCIPLE IS REUSABLE:**
    *a correction that cannot move a number and a correction that moves all of them are
    different acts, and bundling would let the second ride in on the first's evidence.*
    Task 3's change to `CANDIDATE_DROPPED` is provably inert — the member has no producer and
    no committed artifact contains it — and that is precisely what would have carried this one
    through unexamined.
...
20. **WHAT ELSE IS UNIFORM ACROSS ALL SIXTEEN INPUT FIXTURES AND UNCONSTRAINED BY THE CONTRACT?**
    ...
    project used `("time", "y", "x")` while `check_input_contract` requires only that `dims[0]`
    is `time`** — so a name-based implementation was green everywhere and wrong on the first
    real product. That is (i12), and the durable fix was **the first differently-named fixture**
    rather than the corrected call site.

    **THE SAME QUESTION HAS NOT BEEN ASKED OF THE OTHER SHARED PROPERTIES**, and each is a
    candidate for exactly this defect:

    - **coordinate monotonic direction** — every fixture's `y` increases; **a decreasing latitude
      axis is the ordinary case in real altimetry** and appears nowhere here. **This is the one to
      check first**, because a reversed axis produces a *plausible* answer rather than an error.
    - **`source_dtype`** — every fixture is `float32`; the contract admits `float64`, and the
      fingerprint covers the dtype, so a store built from one is not resumable from the other.
    - **chunk layout** — `assembly_spans` splits on chunk boundaries and every fixture is written
      by `to_zarr` with default chunking.
    - **time units and calendar** — every fixture is `datetime64[D]`-derived on a proleptic
      Gregorian calendar; `timeaxis` handles `noleap` and friends, exercised only in
      `tests/test_geometry.py` and not through a run.

    **THE CHECK IS A GREP, NOT A READING**: enumerate what the contract fixes, enumerate what
    every fixture shares, and the difference is the untested freedom. **Cheap to run; the finding
    may not be cheap**, which is why it is a question rather than a task nobody scheduled.

    > **2d WAS OFFERED THIS AND DECLINED IT, 2026-08-30, AND THE REASONING IS THE PART TO KEEP.**
    > 2d builds this project's first deliberately **anisotropic** grid — `32 × 12`, derived from
    > `k` — which is adjacent to this question and **closes no part of it.**
    >
    > **THE ADJACENCY IS WEAKER THAN IT LOOKS.** Grid *shape* and coordinate *direction* are
    > different degrees of freedom. Building `32 × 12` exercises **nothing** about axis direction,
    > so closing the direction half inside 2d would mean adding a fixture 2d has no other use for,
    > tested against machinery 2d is not touching. **That is scope taken on the strength of a
    > shared word rather than a shared mechanism** — and it is this project's own (a5) failure
    > shape pointed at a work item instead of at a decision.
    >
    > **AND CLOSING HALF OF IT WOULD PRODUCE THE WORSE OF TWO STATES.** This question's real
    > subject is `tiling.py`'s four name-based sites, whose closer is a **scope decision** —
    > name-based contract or positional implementation — and that decision is open. **Fixing the
    > fixture gap while the defect stands gives a suite that now covers a case the code still gets
    > wrong**, which is strictly worse than a gap nobody has papered over.
    >
    > **So it stays unowned with the adjacency named**, which is what the record is for.
...
- ~~**2e SHOULD ADD AN `INTERNAL_ERROR` CODE RATHER THAN LIVE WITH THE ALIAS.**~~ **MIGRATED 2026-09-12 — NO LONGER DEFERRED.** It is decision **D3** of [2e's plan](docs/superpowers/plans/2026-09-12-metamer-phase2e.md), owned by its **Task 1**, and the intent is written into **design doc §14.3** as code 5. **The weaker fallback this entry offered — a test asserting exit 1 also asserting the absence of a traceback — was considered and REFUSED**, with the reason recorded at §14.3: it tests the symptom and leaves two different events sharing a code. **Nothing from either is restated here**, and the entry is struck rather than deleted because the hazard it names is the reason Task 1 precedes Task 2.
