...
3. **NEXT ACTION: 2f Task 4 — the primitives sections, and the run's resolved-candidate record.** Tasks 2 and 3 are DONE. **THREE OBLIGATIONS ARE OWED AT TASK 10** and are listed in the open items below; they are the price of 2f having no criteria suite yet. The plan is [`2026-09-19-metamer-phase2f.md`](docs/superpowers/plans/2026-09-19-metamer-phase2f.md), **APPROVED with ten review amendments**; the pre-flight is [`phase2f-preflight.md`](docs/superpowers/notes/phase2f-preflight.md) and every task's entry is written **before** its code. **Task 0 (the clustering spike) and Task 1 (the reader) are DONE.** The queued follow-up is [here](#the-queued-follow-up--2fs-next-commit) and is **SPENT — all five items closed, items 2 and 4 by the follow-up commit.** Both rulings are verbatim at the pre-flight, with an addendum recording the three things the code found that they did not cover.
...
| 3 — the drop row, and 2e's criterion 14 | **DONE.** The row is over pass 1's population, recomputed when the sibling store is present and labelled carried when it is not; **2e's 14 does NOT flip** — forward pointer instead |
| 4–10 | planned |
